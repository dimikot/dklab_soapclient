...
use \DOMDocument, \DOMXPath, \DOMElement, \Exception;
...
class WSSE {
  ...
  /**
   * @var DOMXPath|null $SOAPXPath
   */
  private $SOAPXPath = null;

  /**
   * @var DOMElement $secNode
   */
  ...
  private function locateSecurityHeader( $bMustUnderstand = true, $setActor = null ) {
    if ( $this->secNode == null ) {
      ...
      if ( ! $secnode ) {
        $secnode = $this->soapDoc->createElementNS( WSSE::WSSENS, WSSE::WSSEPFX . ':Security' );
  ...
  public function __construct( \DOMDocument $doc, $bMustUnderstand = true, $setActor = null ) {
    ...
    $this->SOAPXPath->registerNamespace( 'wswsse', WSSE::WSSENS );
  ...
  public function addTimestamp( $secondsToExpire = 3600 ) {
    ...
    $timestamp = $this->soapDoc->createElementNS( WSSE::WSUNS, WSSE::WSUPFX . ':Timestamp' );
    $security->insertBefore( $timestamp, $security->firstChild );
    $currentTime = time();
    $created     = $this->soapDoc->createElementNS( WSSE::WSUNS, WSSE::WSUPFX . ':Created', gmdate( "Y-m-d\TH:i:s", $currentTime ) . 'Z' );
    $timestamp->appendChild( $created );
    if ( ! is_null( $secondsToExpire ) ) {
      $expire = $this->soapDoc->createElementNS( WSSE::WSUNS, WSSE::WSUPFX . ':Expires', gmdate( "Y-m-d\TH:i:s", $currentTime + $secondsToExpire ) . 'Z' );
  ...
  public function addBinaryToken( $cert, $isPEMFormat = true ) {
    ...
    $token = $this->soapDoc->createElementNS( WSSE::WSSENS, WSSE::WSSEPFX . ':BinarySecurityToken', $data );
    $security->insertBefore( $token, $security->firstChild );

    $token->setAttribute( 'EncodingType', 'http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-soap-message-security-1.0#Base64Binary' );
    $token->setAttributeNS( WSSE::WSUNS, WSSE::WSUPFX . ':Id', XMLSecurityDSig::generate_GUID() );
  ...
  public function attachTokentoSig( $token ) {
    ...
    if ( $objDSig = $objXMLSecDSig->locateSignature( $this->soapDoc ) ) {
      $tokenURI = '#' . $token->getAttributeNS( WSSE::WSUNS, "Id" );
      ...
      $tokenRef = $this->soapDoc->createElementNS( WSSE::WSSENS, WSSE::WSSEPFX . ':SecurityTokenReference' );
      $keyInfo->appendChild( $tokenRef );
      $reference = $this->soapDoc->createElementNS( WSSE::WSSENS, WSSE::WSSEPFX . ':Reference' );
  ...
  public function signSoapDoc( XMLSecurityKey $objKey, $options = null ) {
    ...
    $arOptions = array( 'prefix' => WSSE::WSUPFX, 'prefix_ns' => WSSE::WSUNS );
    ...
    if ( is_array( $options ) ) {
      if ( ! empty( $options["KeyInfo"] ) ) {
        if ( ! empty( $options["KeyInfo"]["X509SubjectKeyIdentifier"] ) ) {
          ...
          $tokenRef = $objDoc->createElementNS( WSSE::WSSENS, WSSE::WSSEPFX . ':SecurityTokenReference' );
          $keyInfo->appendChild( $tokenRef );
          $reference = $objDoc->createElementNS( WSSE::WSSENS, WSSE::WSSEPFX . ':KeyIdentifier' );
   ...
   * @param $node
   * @param XMLSecEnc $key
   * @param DOMElement|XMLSecurityKey $token
   * @param array|null $options
   *
   * @return bool
   */
  public function addEncryptedKey( $node, XMLSecEnc $key, DOMElement $token, $options = null ) {
    if ( ! $key->encKey ) {
      return false;
    }
    $encKey   = $key->encKey;
    ...
    $tokenURI  = '#' . $token->getAttributeNS( WSSE::WSUNS, "Id" );
    $reference = $objDoc->createElementNS( WSSE::WSSENS, WSSE::WSSEPFX . ':Reference' );
    $reference->setAttribute( "URI", $tokenURI );
    $tokenRef->appendChild( $reference );
  ...
  public function AddReference( DOMElement $baseNode, $guid ) {
    $refList = null;
